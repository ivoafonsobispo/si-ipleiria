    ...
    public partial class FormMain : Form
    {
        ...
        private void btnGetUsersByCertificate_Click(object sender, EventArgs e)
        {
            using (AuthServiceClient client = new AuthServiceClient())
            {
                ...
                using (X509Certificate2 certificate = new X509Certificate2("si.cert.c.pfx", "ei.si"))
                {
        ...
        private void changeSelectedUserPassword_Click(object sender, EventArgs e)
        {
            ...
            using (X509Certificate2 serverCertificate = new X509Certificate2("si.cert.d.cer"))
            using (AuthServiceClient client = new AuthServiceClient()) {

                #region Encrypt to Server
                ContentInfo contentToEncrypt = new ContentInfo(Encoding.UTF8.GetBytes(password));
                CmsRecipient cmsRecipient = new CmsRecipient(serverCertificate);
                EnvelopedCms envelopedCms = new EnvelopedCms(contentToEncrypt);

                envelopedCms.Encrypt(cmsRecipient);
                byte[] pkcsEncrypted = envelopedCms.Encode();
                #endregion

                #region Sign Data
                ContentInfo contentToSign = new ContentInfo(pkcsEncrypted);
                CmsSigner cmsSigner = new CmsSigner(clientCertificate);
                SignedCms signedCms = new SignedCms(contentToSign);

                signedCms.ComputeSignature(cmsSigner);
                byte[] pkcsSigned = signedCms.Encode();
                #endregion

                string pkcs64 = Convert.ToBase64String(pkcsSigned);

                client.UpdateUserPassword(pkcs64, login);
            }
            ...
            string fileName_a = AppDomain.CurrentDomain.BaseDirectory + @"estg.ei.si.c.pfx";
            string fileName_b = AppDomain.CurrentDomain.BaseDirectory + @"estg.ei.si.d.cer";
