...
namespace AuthService
{
    ...
    public class SqlServerHelper
    {
        ...
        public static int UpdateUserPassword(string login, string password) {
            // Código do professor:
            ...
            try
            {
                ...
                // todo ...
                cmd.CommandText = "UPDATE Users SET password = @passwordHash WHERE login = @login";
                cmd.Parameters.AddWithValue("login", login);

                using (SHA256CryptoServiceProvider sha = new SHA256CryptoServiceProvider())
                {
                    string passwordHash = Convert.ToBase64String(sha.ComputeHash(Encoding.UTF8.GetBytes(password)));
            ...
            try
            {
                ...
                // todo ...
                cmd.CommandText = "UPDATE Users SET password = @passwordHash WHERE id = @id";
        ...
        public static int UserExists(string login, string password)
        {
            ...
            try
            {
                ...
                // attention...
                cmd.CommandText = "SELECT id FROM Users where login = @login AND password = @passwordHash"; // MODIFIED
                cmd.Parameters.AddWithValue("login", login);
        ...
        {
            List<User> users = new List<User>(); // MODIFICADO
            SqlConnection sqlConnection = null;
            try
            {
                sqlConnection = new SqlConnection(ConfigurationManager.ConnectionStrings["ConnectionString"].ConnectionString);
                ...
                while (reader.Read())  // MODIFICADO -> if to while
                {
                    User user = LoadUser(reader);
                    users.Add(user);
                }
